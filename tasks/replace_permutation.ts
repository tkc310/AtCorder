...
class ReplacePermutation {
  ...
  execute({
    ...
    const permutationsSets = this.createSets(permutationsA, rangeN);
  ...
  createSets(permutationA: number[], range: number) {
    // 置き換え済みの順列の配列
    const permutationsSets: number[][] = [];
    [...Array(range)].forEach((_, lIdx) => {
      // lをrange回数繰り返す
      [...Array(range)].forEach((_, rIdx) => {
        // 1 ≤ L ≤ R ≤ N
        if (lIdx > rIdx) return;
        ...
        if (lIdx !== rIdx) {
          // rをrange回数繰り返す
          permutations = this.replacePermutations({
            permutationA,
            l: lIdx,
            r: rIdx,
          });
